...
export default function Products() {
  ...
  async function handleUserInput(e) {
    e.preventDefault();
    try {
      const object = JSON.parse(atob(auth.tokens.access.split('.')[1]))

      const inputObject = {
        // "created_by": toString(object.user_id),
        "created_by": object.user_id,
...
// const token = 'eyJ0eXAiO... /// jwt token';
  // const decoded = jwt_decode(auth.tokens); 
// console.log(777777777777, decoded);
  // console.log(333333333333333, auth.tokens.access)
  // if(auth.tokens.access) {
  //   const object = JSON.parse(atob(auth.tokens.access.split('.')[1]))
  // }
  // console.log(88888888, object.user_id);
